...
BufferGraph PipelineCompiler::makeBufferGraph(BuilderRef b) {
    ...
    for (unsigned i = FirstKernel; i <= LastKernel; ++i) {
        for (const auto e : make_iterator_range(out_edges(i, G))) {
            ...
            if (LLVM_UNLIKELY(Kernel::isLocalBuffer(output))) {
                ...
                bn.Buffer = new ExternalBuffer(0, b, output.getType(), linear, 0);
    ...
    for (const auto e : make_iterator_range(out_edges(PipelineInput, G))) {
        ...
        if (LLVM_LIKELY(bn.Buffer == nullptr)) {
            ...
            bn.Buffer = new ExternalBuffer(0, b, input.getType(), true, 0);
    ...
    for (const auto e : make_iterator_range(in_edges(PipelineOutput, G))) {
        ...
        if (LLVM_LIKELY(bn.Buffer == nullptr)) {
            ...
            bn.Buffer = new ExternalBuffer(0, b, output.getType(), true, 0);
            bn.Type = BufferType::UnownedExternal;
        }
    }
    ...
    for (auto streamSet = FirstStreamSet; streamSet <= LastStreamSet; ++streamSet) {
        ...
        if (LLVM_LIKELY(bn.Buffer == nullptr)) { // is internal buffer
            ...
            assert ((bufferSize % blockWidth) == 0);
            assert ((overflowSize % blockWidth) == 0);
            assert ((underflowSize % blockWidth) == 0);
            if (dynamic || true) {
                buffer = new DynamicBuffer(0, b, baseType, bufferSize / blockWidth, overflowSize / blockWidth, underflowSize / blockWidth, linear, 0U);
            } else {
                buffer = new StaticBuffer(0, b, baseType, bufferSize/ blockWidth, overflowSize / blockWidth, underflowSize / blockWidth, linear, 0U);
            }
            bn.Buffer = buffer;            
        }
    }
