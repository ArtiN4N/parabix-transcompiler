...
void PipelineCompiler::recordStridesPerSegment(BuilderRef b) const {
    if (LLVM_UNLIKELY(DebugOptionIsSet(codegen::TraceStridesPerSegment))) {
        ...
        if (LLVM_UNLIKELY(record == nullptr)) {
            ...
            BasicBlock * const update = BasicBlock::Create(C, "update", record);
        ...
        // b->CallPrintInt("writing " + std::to_string(mKernelIndex), mSegNo);
        assert (mSegNo);
        assert (mTotalNumOfStridesAtExitPhi);
        b->CreateCall(record, { kernelTraceLog, mSegNo, mTotalNumOfStridesAtExitPhi } );

        // b->CallPrintInt("wrote " + std::to_string(mKernelIndex), mTotalNumOfStridesAtExitPhi);
    }
}
