...
void PipelineCompiler::generateInitializeMethod(BuilderRef b) {
    ...
    for (unsigned i = FirstKernel; i <= LastKernel; ++i) {
        ...
        FunctionType * const signalFuncType = cast<FunctionType>(f->getType()->getPointerElementType());
        Value * const signal = b->CreateCall(signalFuncType, f, args);
...
void PipelineCompiler::generateMultiThreadKernelMethod(BuilderRef b) {
    ...
    Value * const processState = allocateThreadState(b, Constant::getNullValue(pThreadTy));
...
void PipelineCompiler::generateFinalizeMethod(BuilderRef b) {
    ...
    for (unsigned i = FirstKernel; i <= LastKernel; ++i) {
        ...
        Value * const f = getKernelFinalizeFunction(b);
        FunctionType * const fFuncType = cast<FunctionType>(f->getType()->getPointerElementType());
        mScalarValue[i] = b->CreateCall(fFuncType, f, params);
    }
...
inline StructType * PipelineCompiler::getThreadStateType(BuilderRef b) const {
    ...
    fields[PROCESS_THREAD_INDEX] = pthreadSelfFn->getReturnType();
...
inline void PipelineCompiler::bindCompilerVariablesToThreadLocalState(BuilderRef b) {
    ...
    Value * const threadLocal = b->getScalarFieldPtr(PIPELINE_THREAD_LOCAL_STATE);
    if (mHasZeroExtendedStream) {
        indices[1] = b->getInt32(ZERO_EXTENDED_BUFFER_INDEX);
        mZeroExtendBuffer = b->CreateInBoundsGEP(threadLocal, indices);
        indices[1] = b->getInt32(ZERO_EXTENDED_SPACE_INDEX);
        mZeroExtendSpace = b->CreateInBoundsGEP(threadLocal, indices);
    }
    ...
    if (canSetTerminateSignal()) {
        if (mNumOfThreads != 1) {
            ...
            mCurrentThreadTerminationSignalPtr = b->CreateInBoundsGEP(threadLocal, indices);
        } else {
    ...
    if (mTarget->hasThreadLocal()) {
        for (unsigned i = FirstKernel; i <= LastKernel; ++i) {
            ...
            if (kernel->hasThreadLocal()) {
                ...
                FunctionType * const fFuncType = cast<FunctionType>(f->getType()->getPointerElementType());
                Value * const handle = b->CreateCall(fFuncType, f, mKernelHandle);
