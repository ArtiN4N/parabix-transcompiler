...
void PipelineCompiler::calculateItemCounts(BuilderRef b) {
    ...
    if (mBoundedKernel) {
        ...
        Value * isFinal = b->CreateIsNull(mNumOfLinearStrides);
...
Value * PipelineCompiler::reserveSufficientCapacity(BuilderRef b, const unsigned outputPort) {
    ...
    const auto size = getCopyBack(streamSet);
    if (size) {
    ...
    Value * const syncLock = nullptr; // getSynchronizationLockPtrForKernel(b, getLastConsumerOfStreamSet(streamSet));

    buffer->reserveCapacity(b, produced, consumed, required, syncLock, mSegNo, mNumOfThreads - 1);

    Value * const newlyWritable = buffer->getLinearlyWritableItems(b, produced, consumed, copyBack);
    #ifdef PRINT_DEBUG_MESSAGES
    debugPrint(b, prefix + "_newlyWritable = %" PRIu64, newlyWritable);
    #endif
...
Value * PipelineCompiler::calculateFinalItemCounts(BuilderRef b, Vec<Value *> & accessibleItems, Vec<Value *> & writableItems) {
    ...
    if (principalFixedRateFactor == nullptr) {
        for (unsigned i = 0; i < numOfInputs; ++i) {
            ...
            if (rate.isFixed()) {
                Value * const fixedRateFactor =
                    b->CreateMulRational(accessibleItems[i], mFixedRateLCM / rate.getRate());
                minFixedRateFactor =
...
//    Value * p = b->CreateSelect(b->CreateICmpUGT(position, ZERO), b->CreateSub(position, ONE), ZERO);

//    Value * const priorPtr = buffer->getRawItemPointer(b, ZERO, p);
//    Value * prior1 = b->CreateLoad(priorPtr);
...
//    args[1] = b->GetString("popcount[%" PRIx64 "] => %" PRIu64 "\n");
//    args[2] = priorPtr;
//    args[3] = prior1;
...
Value * PipelineCompiler::getMaximumNumOfPartialSumStrides(BuilderRef b, const StreamSetPort port) {
    ...
    Value * const noMore = b->CreateICmpEQ(strideIndex, ZERO);
    Value * const done = b->CreateOr(hasEnough, noMore);


//    FixedArray<Value *, 4> args;
//    args[0] = b->getInt32(STDERR_FILENO);
//    args[1] = b->GetString("popcount[%" PRIu64 "] = %" PRIu64 " ?= %" PRIu64 "\n");
//    args[2] = strideIndex;
//    args[3] = requiredItems;
//    args[4] = initialItemCount;
//    b->CreateCall(b->GetDprintf(), args);
    ...
    if (LLVM_UNLIKELY(mCheckAssertions)) {
        ...
        b->CreateAssert(b->CreateOr(b->CreateICmpUGE(numOfStrides, STEP), done),
