...
void ExpandKernel::generateMultiBlockLogic(KernelBuilder & b, Value * const numOfStrides) {
    ...
    for (unsigned i = 0; i < inputPacksPerStride; i++) {
        ...
        //expanded[2*i] = b.CreateZExt(bytepack[i], b.getInt16Ty());
        //expanded[2*i+1] = b.CreateSExt(bytepack[i], b.getInt16Ty());
        
        Value * expand16h = b.CreateZExt(bytepack[i], FixedVectorType::get(b.getInt16Ty(), 32));
        Value * expand16l = b.CreateSExt(bytepack[i], FixedVectorType::get(b.getInt16Ty(), 32));

        expanded[2*i] = b.CreateBitCast(expand16h, FixedVectorType::get(b.getInt8Ty(), 64));
        expanded[2*i+1] = b.CreateBitCast(expand16l, FixedVectorType::get(b.getInt8Ty(), 64));
