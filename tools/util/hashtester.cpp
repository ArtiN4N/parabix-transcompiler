...
static cl::opt<unsigned> NumOfBuckets("buckets",  cl::init(100),
                                      cl::desc("number of hash buckets used for chi-squared test (default=100)"),
...
public:
    BixHashGenerator(BuilderRef b, const BixHashGraphGenome & genome, StreamSet * basis, StreamSet * LineFeeds, StreamSet * hashes, StreamSet * selector_span)
...
private:
    BixHashGenerator(BuilderRef b, const BixHashGraphGenome & genome, std::string && signature, StreamSet * basis, StreamSet * LineFeeds, StreamSet * hashes, StreamSet * selector_span)
    : PabloKernel(b, "bhg" + getStringHash(signature),
                  {Binding{"basis", basis}, Binding{"lf", LineFeeds}},
...
void BixHashGenerator::generatePabloMethod() {
    ...
    PabloAST * run = pb.createNot(getInputStreamSet("lf")[0]);
    ...
    for (unsigned i = 0; i < m; ++i) {
        assert (in_degree(i + NumOfBasisBits, mGenome) > 0);
    ...
    for (unsigned i = 0; i < steps; ++i) {
        ...
        for (unsigned j = 0; j < m; j++) {
            assert (in_degree(s + j, mGenome) > 0);
            //assert (out_degree(s + j, mGenome) > 0 || i == (steps - 1));
...
public:
    ...
    HashTable()
    : mTable(NumOfBuckets)
    , mBitFlipHistory(NumOfBitFlips + 1)
    , mHashCodeIndex(0) {
        for (auto & T : mTable) {
            T = 0;
        }
    ...
    double calculateScore() const {
        ...
        assert (0.0 <= p && p <= 2.0);
        return chiSquareTest() / std::pow(2.0, 2.0 - p);
    }
    ...
    double percentDifference() const {
        ...
        const double a = std::abs<double>(2 * (mNumOfSameBitValues - mNumOfDifferentBitValues));
...
HashDemoFunctionType hashdemo_gen (CPUDriver & driver, const BixHashGraphGenome & genome) {
    ...
    P->CreateKernelFamilyCall<BixHashGenerator>(genome, u8basis, LineFeeds, BixHashes, SelectorSpans);

//    P->CreateKernelCall<DebugDisplayKernel>("BixHashes", SelectorSpans);
...
//    P->CreateKernelCall<DebugDisplayKernel>("SelectorSpans", SelectorSpans);

//    P->CreateKernelCall<DebugDisplayKernel>("symbolEnds", SymbolEnds);

    StreamSet * masked = P->CreateStreamSet(NumOfHashBits);
    P->CreateKernelCall<MaskHash>(BixHashes, SymbolEnds, masked);

//    P->CreateKernelCall<DebugDisplayKernel>("masked", masked);

//    P->CreateKernelCall<DebugDisplayKernel>("hashes", Hashes);
...
public:
    ...
    auto recombine(const individual_type & x, const individual_type & y,
        ...
        auto fixChild = [&](BixHashGraphGenome & X) {
    ...
    auto mutate(individual_type & x, generator_type& g) const -> void {
        ...
        std::binomial_distribution<size_t> P(n / 8, 0.2);
        ...
        for (unsigned p = 0; p < count; ++p) {
            ...
            for (unsigned i = 0; i <= steps; ++i) {
                ...
                for (unsigned j = s; j < e; ++j) {
                    const auto l = (i == 0) ? n : m;
                    assert (l > 0);
                    std::binomial_distribution<size_t> M(l - 1, 0.2);
                    const auto m = M(rng);
                    for (auto k = 0; k <= m; ++k) {
            ...
            #ifndef NDEBUG
            for (unsigned i = NumOfBasisBits; i < size; ++i) {
                assert (in_degree(i, *C) > 0);
            }
            #endif
...
void runMurmur3HashOnData(const std::vector<char> & data) {
    ...
    const size_t mask = (1ULL << NumOfHashBits) - 1ULL;
...
int main(int argc, char *argv[]) {
    ...
    const unsigned generation_count = 500;
    const unsigned population_count = 20;
