...
HexLinesFunctionType generatePipeline(CPUDriver & pxDriver) {
    ...
    //  The HLFWDTH positions need to be expanded to generate two hex digits each.
    ...
    std::vector<re::CC *> HLFWDTH_CC = {re::makeCC(re::makeByte(0x21, 0x7d), re::makeByte(0x7d, 0x7e))};
    ...
    StreamSet * hexInsertMask = InsertionSpreadMask(P, HLFWDTH, InsertPosition::After);
