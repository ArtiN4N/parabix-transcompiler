...
void PackKernel::generateMultiBlockLogic(KernelBuilder & b, Value * const numOfStrides) {
    ...
    llvm::errs() << "This is ExpandLoop: " << packLoop << "\n"; // for testing
    llvm::errs() << "This is expandFinalize: " << packFinalize << "\n"; // for testing
    ...
    llvm::errs() << "Block Offset Phi: " << blockOffsetPhi << "\n";
    ...
    for (unsigned i = 0; i < outputPacksPerStride; i++) {
        ...
        // Print actual packed values for debugging
        llvm::errs() << "packed[" << i << "]: " << *packed[i] << "\n";
    }
