...
void preprocessPipeline(ParabixDriver & pxDriver){
    ...
    kernel::Kernel * sourceK = pxDriver.addKernelInstance(std::make_unique<kernel::MemorySourceKernel>(iBuilder, iBuilder->getInt8PtrTy(), segmentSize));
    sourceK->setInitialArguments({inputStream, fileSize});
    pxDriver.makeKernelCall(sourceK, {}, {ByteStream});

    StreamSetBuffer * MatchResults = pxDriver.addBuffer(std::make_unique<CircularBuffer>(iBuilder, iBuilder->getStreamSetTy(1, 1), segmentSize * bufferSegments));
    kernel::Kernel * linefeedK = pxDriver.addKernelInstance(std::make_unique<kernel::DirectCharacterClassKernelBuilder>(iBuilder, "linefeed", std::vector<re::CC *>{re::makeCC(0x0A)}, 1));
    pxDriver.makeKernelCall(linefeedK, {ByteStream}, {MatchResults});
    
    kernel::Kernel * scanMatchK = pxDriver.addKernelInstance(std::make_unique<kernel::CCScanKernel>(iBuilder, 1));
